...
class OnlineTest extends JFrame implements ActionListener
{
	...
	OnlineTest(String s)
	{
		super(s); //label of page
		l=new JLabel(); //new object of label
		add(l); //added label to page
		bg=new ButtonGroup(); //group of buttons
		for(int i=0;i<5;i++)
		{
			jb[i]=new JRadioButton();//new button 	
			add(jb[i]); // add button
			bg.add(jb[i]); 
		}
		b1=new JButton("Next");
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getActionCommand().equals("Bookmark"+y))
		{
			...
			current=m[y];
			set();
			((JButton)e.getSource()).setEnabled(false);//downcast current bookmark button
			current=now;
		}
		}
